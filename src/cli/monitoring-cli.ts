...
class MonitoringCLI {
  ...
  async startMonitoring(options: MonitoringCLIOptions = {}): Promise<void> {
    ...
    try {
      ...
      if (options.port) {
        config.metricsConfig = { ...config.metricsConfig, prometheusPort: options.port };
      }
      if (options.healthPort) {
        config.healthConfig = { ...config.healthConfig, port: options.healthPort };
      }
      if (options.dashboardPort) {
        config.dashboardConfig = { ...config.dashboardConfig, port: options.dashboardPort };
      }
   ...
   */
  private async loadConfig(configPath?: string): Promise<MonitoringServiceConfig> {
  ...
  private setupEventListeners(): void {
    ...
    this.monitoringService.on('alert', (alert) => {
      ...
      const colorFunc = severityColor === 'blue' ? chalk.blue :
  ...
  private displayMetrics(metrics: PerformanceMetrics): void {
    const sections = [
      ...
      },
      {
        ...
        items: [
          ['Memory (Heap)', `${Math.round(metrics.memoryUsage.heapUsed / 1024 / 1024)}MB`],
          ['Memory (Total)', `${Math.round(metrics.memoryUsage.heapTotal / 1024 / 1024)}MB`],
          ['Memory (RSS)', `${Math.round(metrics.memoryUsage.rss / 1024 / 1024)}MB`],
  ...
  private displayHealth(health: SystemHealth): void {
    ...
    console.log(`  Avg Response Time: ${health.metrics.averageResponseTime.toFixed(1)}ms`);
  }
  ...
  private displayAlerts(alerts: AlertNotification[], severityFilter?: string): void {
    ...
    filteredAlerts.forEach(alert => {
      ...
      const colorFunc2 = severityColor === 'blue' ? chalk.blue :
                       severityColor === 'yellow' ? chalk.yellow :
                       severityColor === 'magenta' ? chalk.magenta :
                       severityColor === 'red' ? chalk.red : chalk.gray;
      console.log(colorFunc2(`[${alert.severity.toUpperCase()}] ${alert.title}`));
  ...
  private displayPerformanceReport(report: PerformanceReport): void {
    ...
    console.log(`  Requests/sec: ${report.metrics.requestsPerSecond.toFixed(1)}`);
