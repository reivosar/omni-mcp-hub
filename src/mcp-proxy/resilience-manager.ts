...
export class ResilienceManager extends EventEmitter {
  ...
  constructor(config: Partial<SystemResilienceConfig> = {}, logger?: ILogger) {
    super();
  ...
  async addServer(
    ...
    if (this.connections.has(serverConfig.name)) {
      this.logger.warn(
        `[RESILIENCE-MGR] Server ${serverConfig.name} already exists`,
      );
      return;
    }
    ...
    if (
      ...
      this.config.resourceManagement.maxTotalConnections
    ) {
      throw new Error(
        `Maximum connection limit reached (${this.config.resourceManagement.maxTotalConnections})`,
      );
    }

    this.logger.info(
      `[RESILIENCE-MGR] Adding resilient server: ${serverConfig.name}`,
    );

    const connection = new ResilientMCPConnection(
      serverConfig,
      resilienceConfig,
      this.logger,
    );
    ...
    try {
      ...
      this.logger.info(
        `[RESILIENCE-MGR] Successfully added resilient server: ${serverConfig.name}`,
      );
      this.emit("serverAdded", serverConfig.name);
    } catch (error) {
      this.logger.error(
        `[RESILIENCE-MGR] Failed to connect to ${serverConfig.name}:`,
        error,
      );
      this.createAlert(
        "error",
        `Failed to connect to server ${serverConfig.name}`,
        serverConfig.name,
  ...
  async removeServer(serverName: string): Promise<void> {
    ...
    try {
      ...
      this.logger.info(
        `[RESILIENCE-MGR] Successfully removed server: ${serverName}`,
      );
      this.emit("serverRemoved", serverName);
    } catch (error) {
      this.logger.error(
        `[RESILIENCE-MGR] Error removing server ${serverName}:`,
        error,
      );
    }
  }
  ...
  private async executeWithLoadBalancing<T>(
    ...
    if (
      this.requestQueue.length >= this.config.resourceManagement.maxQueueSize
    ) {
  ...
  private async selectServer(
    preferredServer?: string,
  ): Promise<ResilientMCPConnection | null> {
    const availableConnections = Array.from(this.connections.entries()).filter(
      ([, connection]) => {
        ...
        return (
          ...
          stats.currentOperations <
            this.config.loadBalancing.maxConcurrentRequests
        );
      },
    );
  ...
  private selectLeastConnections(
    ...
    return connections.reduce((best, current) => {
      ...
      return currentStats.currentOperations < bestStats.currentOperations
        ? current
        : best;
    })[1];
  }
  ...
  private selectLeastResponseTime(
    ...
    return connections.reduce((best, current) => {
      const bestStats = best[1].getStats();
      const currentStats = current[1].getStats();
      return currentStats.averageOperationTimeMs <
        bestStats.averageOperationTimeMs
        ? current
        : best;
    })[1];
  }
  ...
  private selectHealthWeighted(
    ...
    const totalHealth = healthScores.reduce(
  ...
  private handleConnectionStateChange(
    ...
    this.logger.info(
      `[RESILIENCE-MGR] Server ${serverName} state: ${oldState} -> ${newState}`,
    );
    ...
    switch (newState) {
      ...
      case ConnectionState.CIRCUIT_OPEN:
        this.createAlert(
          "warning",
          `Circuit breaker opened for ${serverName}`,
          serverName,
        );
        break;
      ...
      case ConnectionState.DEGRADED:
        this.createAlert(
          "warning",
          `Server ${serverName} entered degraded mode`,
          serverName,
        );
        break;
    }
  ...
  private async triggerFailover(failedServerName: string): Promise<void> {
    this.logger.warn(
      `[RESILIENCE-MGR] Triggering failover for ${failedServerName}`,
    );
  ...
  private scheduleRecovery(serverName: string, delayMs: number): void {
    ...
    this.logger.info(
      `[RESILIENCE-MGR] Scheduling recovery for ${serverName} in ${delayMs}ms`,
    );
  ...
  private async attemptRecovery(serverName: string): Promise<void> {
    ...
    try {
      ...
      if (!connection) {
        this.logger.warn(
          `[RESILIENCE-MGR] Connection ${serverName} not found for recovery`,
        );
        return;
      }
      ...
      if (this.config.failover.healthCheckBeforeFailback) {
        ...
        if (!isHealthy) {
          this.logger.warn(
            `[RESILIENCE-MGR] Pre-recovery health check failed for ${serverName}`,
          );
          this.scheduleRecovery(
            serverName,
      ...
      this.logger.error(
        `[RESILIENCE-MGR] Recovery failed for ${serverName}:`,
        error,
      );
      this.scheduleRecovery(
        serverName,
  ...
  private startMonitoring(): void {
    ...
    this.logger.debug(
      `[RESILIENCE-MGR] Started monitoring (interval: ${this.config.monitoring.metricsIntervalMs}ms)`,
    );
  }
  ...
  private startRecoveryManager(): void {
    ...
    this.logger.debug(
      `[RESILIENCE-MGR] Started recovery manager (interval: ${this.config.recovery.recoveryIntervalMs}ms)`,
    );
  }
  ...
  private checkAlertConditions(): void {
    ...
    if (this.systemMetrics.errorRatePercent > thresholds.errorRatePercent) {
      this.createAlert(
        ...
        undefined,
        {
          errorRate: this.systemMetrics.errorRatePercent,
    ...
    if (this.systemMetrics.averageResponseTimeMs > thresholds.responseTimeMs) {
      this.createAlert(
        ...
        undefined,
        {
          responseTime: this.systemMetrics.averageResponseTimeMs,
    ...
    const unhealthyPercent =
      this.systemMetrics.totalServers > 0
        ? (this.systemMetrics.unhealthyServers /
            this.systemMetrics.totalServers) *
          100
        : 0;
  ...
  private async performRecoveryCheck(): Promise<void> {
    if (
      this.activeRecoveries.size >= this.config.recovery.maxParallelRecoveries
    ) {
      return;
    }
    ...
    for (const [serverName] of failedServers.slice(
      0,
  ...
  private updateResponseMetrics(responseTime: number, success: boolean): void {
    ...
    this.systemMetrics.averageResponseTimeMs =
      this.systemMetrics.averageResponseTimeMs === 0
        ? responseTime
        : this.systemMetrics.averageResponseTimeMs * (1 - alpha) +
          responseTime * alpha;
  }
  ...
  private createAlert(
    ...
    if (this.config.monitoring.enableDetailedLogging) {
      this.logger.warn(
        `[RESILIENCE-MGR] ALERT [${severity.toUpperCase()}]: ${message}`,
        details,
      );
    }
  }
    ...
    const disconnectPromises = Array.from(this.connections.values()).map(
      (connection) =>
        connection
          ...
          .catch((error) =>
            this.logger.error(
              "[RESILIENCE-MGR] Error during shutdown disconnect:",
              error,
            ),
          ),
    );
