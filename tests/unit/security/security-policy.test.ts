import { SecurityPolicyManager, SecurityPolicy, DEFAULT_SECURITY_POLICY } from '../../../src/security/security-policy';
...
describe('SecurityPolicyManager', () => {
  ...
  describe('Policy Management', () => {
    test('should update policy partially', () => {
      ...
      expect(policy.auditEnabled).toBe(DEFAULT_SECURITY_POLICY.auditEnabled); // Should remain unchanged
    });
    ...
    test('should not mutate original policy when getting copy', () => {
      ...
      expect(policy2.sandboxEnabled).toBe(DEFAULT_SECURITY_POLICY.sandboxEnabled);
    });
  });
  ...
  describe('Command Validation', () => {
    ...
    test('should block explicitly blocked commands even if in allowed list', () => {
      // Add a command to both allowed and blocked lists
      policyManager.updatePolicy({
        allowedCommands: [...DEFAULT_SECURITY_POLICY.allowedCommands, 'rm'],
        blockedCommands: [...DEFAULT_SECURITY_POLICY.blockedCommands, 'rm']
      });
  ...
  describe('Argument Validation', () => {
    ...
    test('should validate each argument independently', () => {
      const mixedArgs = ['safe-arg', '--force']; // One safe, one dangerous
  ...
  describe('Security Policy Configuration', () => {
    test('should maintain security when updating allowed commands', () => {
      // Try to add dangerous commands
      policyManager.updatePolicy({
        allowedCommands: ['python', 'rm', 'sudo', 'dd']
      });

      // Blocked commands should still prevent execution
      expect(policyManager.isCommandAllowed('rm')).toBe(false);
    ...
    test('should validate policy changes maintain security', () => {
      // Test that critical security settings cannot be bypassed
      policyManager.updatePolicy({
        blockedCommands: [], // Try to remove all blocked commands
        allowedCommands: ['rm', 'sudo', 'dd'] // Try to allow dangerous commands
      });

      // The policy should still prevent dangerous operations
      const policy = policyManager.getPolicy();
      expect(policy.blockedCommands).toEqual([]); // Update applied
      expect(policy.allowedCommands).toContain('rm'); // Update applied
      
      // But validation should still catch dangerous commands through other means
  ...
  describe('Edge Cases', () => {
    ...
    test('should handle special characters in commands', () => {
      ...
      for (const command of specialCommands) {
        // These should be handled based on their base name
        const isAllowed = DEFAULT_SECURITY_POLICY.allowedCommands.some(allowed => 
    ...
    test('should handle very long arguments', () => {
      ...
      // Should be handled gracefully (either allowed if safe or blocked if detected as suspicious)
  ...
  describe('Default Policy Validation', () => {
    ...
    test('should include essential safe commands', () => {
      ...
      for (const command of essentialCommands) {
        expect(DEFAULT_SECURITY_POLICY.allowedCommands).toContain(command);
      }
    });
    ...
    test('should block dangerous commands', () => {
      ...
      for (const command of dangerousCommands) {
        expect(DEFAULT_SECURITY_POLICY.blockedCommands).toContain(command);
      }
    });
    ...
    test('should block system directories', () => {
      ...
      for (const dir of systemDirs) {
        expect(DEFAULT_SECURITY_POLICY.blockedPaths).toContain(dir);
      }
    });
  });
});