...
describe('MemoryCache', () => {
    let cache;
    beforeEach(() => {
        cache = new cache_1.MemoryCache(1);
    });
    afterEach(() => {
        cache.destroy();
        jest.useRealTimers();
    ...
    describe('Constructor', () => {
        it('should initialize with default TTL', () => {
            const defaultCache = new cache_1.MemoryCache();
            expect(defaultCache).toBeInstanceOf(cache_1.MemoryCache);
            defaultCache.destroy();
        });
        it('should initialize with custom TTL', () => {
            const customCache = new cache_1.MemoryCache(120);
            expect(customCache).toBeInstanceOf(cache_1.MemoryCache);
            customCache.destroy();
        });
        it('should not start cleanup interval in test environment', () => {
            process.env.NODE_ENV = 'test';
            const testCache = new cache_1.MemoryCache();
            expect(testCache).toBeInstanceOf(cache_1.MemoryCache);
            testCache.destroy();
        });
        it('should start cleanup interval in non-test environment', () => {
            const originalEnv = process.env.NODE_ENV;
            process.env.NODE_ENV = 'production';
            const prodCache = new cache_1.MemoryCache();
            expect(prodCache).toBeInstanceOf(cache_1.MemoryCache);
            prodCache.destroy();
...
describe('CacheManager', () => {
    ...
    let mockCache;
    beforeEach(() => {
        mockCache = new cache_1.MemoryCache();
        cacheManager = new cache_1.CacheManager(mockCache);
    });
    afterEach(() => {
        mockCache.destroy();
    });
    describe('Constructor', () => {
        it('should initialize with provided cache', () => {
            expect(cacheManager).toBeInstanceOf(cache_1.CacheManager);
        });
        it('should initialize with default MemoryCache when no cache provided', () => {
            const defaultManager = new cache_1.CacheManager();
            expect(defaultManager).toBeInstanceOf(cache_1.CacheManager);
        });
    });
    ...
    describe('cache statistics', () => {
        ...
        test('should return stats for unknown cache type', () => {
            const mockCacheInterface = {
                get: jest.fn(),
                set: jest.fn(),
                delete: jest.fn(),
                clear: jest.fn()
            };
            const manager = new cache_1.CacheManager(mockCacheInterface);
    ...
    describe('Error Handling', () => {
        let mockCacheInterface;
        let errorCacheManager;
        beforeEach(() => {
            mockCacheInterface = {
                get: jest.fn(),
                set: jest.fn(),
            ...
            errorCacheManager = new cache_1.CacheManager(mockCacheInterface);
        });
...
describe('CacheInterface Compliance', () => {
    test('should implement CacheInterface correctly', () => {
        const cache = new cache_1.MemoryCache();
