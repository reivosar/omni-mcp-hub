...
export class ProcessErrorHandler {
  ...
  private handleSignal(signal: string): void {
    this.logger.info(
      `[PROCESS-SIGNAL] Received ${signal}, initiating graceful shutdown`,
    );
  ...
  private performGracefulShutdown(reason: string, exitCode: number): void {
    ...
    this.logger.info(
      `[SHUTDOWN] Starting graceful shutdown (reason: ${reason})`,
    );
  ...
  startMetricsCollection(intervalMs: number = 60000): NodeJS.Timeout {
    this.logger.info(
      `[PROCESS-METRICS] Starting metrics collection (interval: ${intervalMs}ms)`,
    );
