...
describe('Error Handling and Edge Cases', () => {
    ...
    beforeEach(() => {
        ...
        server = new mcp_sse_server_1.MCPSSEServer(port);
        app = server.app;
    ...
    describe('File Not Found Errors', () => {
        test('should handle repository not found', async () => {
            ...
            mockGitHubAPI.listFiles = jest.fn().mockRejectedValue(new Error('Repository nonexistent/repo not found or branch main does not exist'));
        ...
        test('should handle branch not found', async () => {
            const originalListFiles = mockGitHubAPI.listFiles;
            mockGitHubAPI.listFiles = jest.fn().mockRejectedValue(new Error('Repository testorg/testrepo not found or branch nonexistent does not exist'));
            const response = await (0, supertest_1.default)(app)
                .post('/sse')
                .send({
                jsonrpc: '2.0',
        ...
        test('should handle empty repository (no CLAUDE.md files)', async () => {
            mockGitHubAPI.setMockFiles('empty', 'repo', 'main', []);
            const response = await (0, supertest_1.default)(app)
                .post('/sse')
                .send({
                jsonrpc: '2.0',
    ...
    describe('Authentication and Rate Limiting', () => {
        ...
        test('should handle authentication failures', async () => {
            const originalListFiles = mockGitHubAPI.listFiles;
            mockGitHubAPI.listFiles = jest.fn().mockRejectedValue(new Error('GitHub API error: 401 Unauthorized'));
            const response = await (0, supertest_1.default)(app)
                .post('/sse')
                .send({
                jsonrpc: '2.0',
...
`;
            mockGitHubAPI.setMockFiles('testorg', 'testrepo', 'main', ['CLAUDE.md']);
            mockGitHubAPI.setMockFileContent('testorg', 'testrepo', 'CLAUDE.md', 'main', claudeContent);
            const scope = (0, nock_1.default)('https://example.com')
        ...
        test('should handle malformed external URLs', async () => {
            ...
            const malformedUrls = externalEvents.filter(e => e.data.params.url === 'not-a-url' || e.data.params.url === 'ftp://invalid.url/doc.md');
    ...
    describe('Internal Server Errors', () => {
        ...
        test('should handle concurrent request errors', async () => {
            mockGitHubAPI.setMockFiles('testorg', 'testrepo', 'main', ['CLAUDE.md']);
            mockGitHubAPI.setMockFileContent('testorg', 'testrepo', 'CLAUDE.md', 'main', 'Content');
            const promises = Array.from({ length: 3 }, (_, i) => (0, supertest_1.default)(app)
                .post('/sse')
                .timeout(5000)
                .send({
                jsonrpc: '2.0',
