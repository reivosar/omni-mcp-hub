...
import {
  ...
  VersionedConfig,
} from "./schema-version-manager.js";
...
export class ClaudeConfigManager {
    ...
    try {
      ...
      if (this.configCache.has(absolutePath)) {
        ...
        try {
          ...
          const cacheTime =
            cached._lastModified && typeof cached._lastModified === "string"
      ...
      if (compatInfo.requiresMigration) {
        this.logger.info(
          `Config at ${filePath} requires migration from v${this.versionManager.extractVersion(config).major}.${this.versionManager.extractVersion(config).minor}.${this.versionManager.extractVersion(config).patch}`,
        );
        ...
        if (options.autoMigrate !== false) {
          ...
          this.logger.info(
            `Config successfully migrated and saved to ${absolutePath}`,
          );
        } else {
          this.logger.warn(
            `Config migration required but autoMigrate is disabled. Use with autoMigrate: true to perform migration.`,
          );
        }
      }
  ...
  private configToClaudeFormat(
    ...
    for (const key of simpleKeys) {
      if (config[key] && typeof config[key] === "string") {
        lines.push(
          `${key.replace("_", " ").replace(/\b\w/g, (l) => l.toUpperCase())}: ${config[key]}`,
        );
      }
    }
    ...
    if (config.inheritance && !skipInheritance) {
      ...
      if (inheritance.allowToolsAppend?.length) {
        lines.push(
          `Allow Tools Append: ${inheritance.allowToolsAppend.join(", ")}`,
        );
      }
      if (inheritance.sectionAppendList?.length) {
        lines.push(
          `Section Append List: ${inheritance.sectionAppendList.join(", ")}`,
        );
      }
      lines.push("");
    }
   ...
   */
  async checkConfigVersion(filePath: string): Promise<{
    version: string;
    compatible: boolean;
  ...
  async listAvailableConfigs(
    directory: string,
  ): Promise<Array<{ profileName: string; filePath: string }>> {
    try {
  ...
  mergeConfigurations(
    ...
    arrayFields.forEach((field) => {
      if (override[field] && Array.isArray(override[field])) {
        ...
        merged[field] = (sourceArray as unknown[]).concat(
          overrideArray as unknown[],
        );
      }
    });
