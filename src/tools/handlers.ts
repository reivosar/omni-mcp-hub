...
export class ToolHandlers {
  ...
  constructor(
    ...
    this.logger.debug(
      "[TOOL-HANDLERS] Active profiles count:",
      activeProfiles.size,
    );
    this.logger.debug(
      "[TOOL-HANDLERS] Proxy manager provided:",
      !!proxyManagerOrFileScanner,
    );
    ...
    if (proxyManagerOrFileScanner && "addServer" in proxyManagerOrFileScanner) {
      this.logger.debug(
        "[TOOL-HANDLERS] Using new signature with MCPProxyManager",
      );
  ...
  private setupListToolsHandler(): void {
    this.server.setRequestHandler(ListToolsRequestSchema, async () => {
      ...
      const pathResolver = PathResolver.getInstance();
      const yamlConfigPath = pathResolver.getYamlConfigPath();
      const yamlConfigManager = YamlConfigManager.createWithPath(
        yamlConfigPath,
        this.logger,
      );
      const config = yamlConfigManager?.getConfig?.();
      const hasFileSettings = !!(
        config?.fileSettings && Object.keys(config.fileSettings).length > 0
      );
      ...
      if (this.proxyManager) {
        ...
        this.logger.info(
          `[TOOL-HANDLER] Connected servers count: ${connectedServers.length}`,
        );
        this.logger.info(
          `[TOOL-HANDLER] FileSettings configured: ${hasFileSettings}`,
        );
      } else {
        this.logger.info(
          `[TOOL-HANDLER] No proxy manager available - defaulting to local resources mode`,
        );
      }

      this.logger.info(
        `[TOOL-HANDLER] hasLocalResources determined: ${hasLocalResources}`,
      );

      this.logger.debug(
        `[TOOL-HANDLER] Local resources configured: ${hasLocalResources}`,
      );
      ...
      const baseTools = hasLocalResources
        ? [
              ...
              inputSchema: {
                ...
                properties: {
                  ...
                  autoApply: {
                    type: "boolean",
                    description:
                      "Automatically apply the configuration after loading (default: true)",
                  },
                },
                required: [],
              },
            },
            {
              ...
              description:
                "List all CLAUDE.md configuration files (both loaded and available)",
              inputSchema: {
                type: "object",
                properties: {},
                required: [],
            ...
            },
            {
              ...
              description:
                "Get information about the currently applied configuration",
              inputSchema: {
                type: "object",
                properties: {},
                required: [],
      ...
      this.logger.debug(
        `[TOOL-HANDLER] Checking proxy manager: ${this.proxyManager ? "exists" : "null"}`,
      );

      if (this.proxyManager) {
        this.logger.debug(
          `[TOOL-HANDLER] Getting external tools from proxy manager...`,
        );
        this.logger.debug(
          `[TOOL-HANDLER] Proxy manager connected servers:`,
          this.proxyManager.getConnectedServers(),
        );

        const externalTools = this.proxyManager.getAggregatedTools();
        this.logger.debug(
          `[TOOL-HANDLER] External tools count: ${externalTools.length}`,
        );

        if (externalTools.length === 0) {
          this.logger.debug(
            `[TOOL-HANDLER] WARNING: No external tools found - checking proxy manager state...`,
          );
          this.logger.debug(
            `[TOOL-HANDLER] Proxy manager has ${this.proxyManager.getConnectedServers().length} connected servers`,
          );
        }

        externalTools.forEach((tool, i) => {
          this.logger.debug(
            `[TOOL-HANDLER] External tool ${i + 1}: ${tool.name} - ${tool.description}`,
          );
        });

        aggregatedTools = [...aggregatedTools, ...externalTools];
        this.logger.debug(
          `[TOOL-HANDLER] Total aggregated tools count: ${aggregatedTools.length}`,
        );
      } else {
        this.logger.debug(
          `[TOOL-HANDLER] No proxy manager available - returning base tools only`,
        );
      }
      ...
      aggregatedTools.forEach((tool, i) => {
        this.logger.debug(
          `[TOOL-HANDLER] Final tool ${i + 1}: ${(tool as Record<string, unknown>).name}`,
        );
      });
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      switch (name) {
        ...
        default:
          ...
          if (this.proxyManager) {
            this.logger.debug(
              `[TOOL-HANDLER] Attempting proxy tool call: ${name}`,
            );
  ...
  private async doHandleApplyClaudeConfig(args: unknown) {
    this.logger.debug(
      "[APPLY-CLAUDE-CONFIG] Starting configuration application",
    );
    this.logger.debug(
      "[APPLY-CLAUDE-CONFIG] Raw arguments:",
      JSON.stringify(args),
    );
    ...
    if (typeof args === "string") {
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] Arguments type: string, treating as profileName",
      );
      // Treat as profileName if string (common usage pattern)
      profileName = args;
    } else if (args && typeof args === "object") {
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] Arguments type: object, parsing properties",
      );
      ...
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] Parsed - filePath:",
        filePath,
        "profileName:",
        profileName,
        "autoApply:",
      ...
      if (!filePath && !profileName) {
        ...
        if (keys.length > 0) {
          const firstValue =
            ((args as Record<string, unknown>)[keys[0]] as string) || "";
          if (keys[0] === "filePath") {
            filePath = firstValue;
          } else {
    ...
    this.logger.debug(
      "[APPLY-CLAUDE-CONFIG] After parsing - filePath:",
      filePath,
      "profileName:",
      profileName,
    );
    ...
    if (!filePath && profileName) {
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] Handling profileName-only case:",
        profileName,
      );
      // First check if profile is already loaded
      if (this.activeProfiles.has(profileName)) {
        this.logger.debug(
          "[APPLY-CLAUDE-CONFIG] Profile already loaded:",
          profileName,
        );
        ...
        if (autoApply) {
          const behaviorInstructions = BehaviorGenerator.generateInstructions(
            config!,
          );
          ...
          if (
            this.omniMCPServer &&
            typeof this.omniMCPServer.setAppliedBehaviorInstructions ===
              "function"
          ) {
            this.omniMCPServer.setAppliedBehaviorInstructions(
              behaviorInstructions,
            );
            this.logger.info(
              "[APPLY-CLAUDE-CONFIG] Behavior instructions applied to system for profile:",
              profileName,
            );
          }
          ...
          responseMessages.push({
            type: "text" as const,
            text: `Configuration includes: ${Object.keys(config!)
      ...
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] Profile not loaded, searching for file path",
      );
      const existingConfig = this.activeProfiles.get(profileName);
      const existingPath = (existingConfig as Record<string, unknown>)
        ?._filePath as string;
      if (existingPath) {
        this.logger.debug(
          "[APPLY-CLAUDE-CONFIG] Found existing path:",
          existingPath,
        );
      ...
      if (!filePath) {
        this.logger.debug(
          "[APPLY-CLAUDE-CONFIG] No existing path found, checking YAML config autoLoad profiles",
        );
        const pathResolver = PathResolver.getInstance();
        const yamlConfigPath = pathResolver.getAbsoluteYamlConfigPath();
        this.logger.debug(
          "[APPLY-CLAUDE-CONFIG] YAML config path:",
          yamlConfigPath,
        );
        const yamlConfigManager = YamlConfigManager.createWithPath(
          yamlConfigPath,
        ...
        try {
          ...
          this.logger.debug(
            "[APPLY-CLAUDE-CONFIG] Loaded YAML config, autoLoad profiles:",
            yamlConfig.autoLoad?.profiles?.length || 0,
          );
          const autoLoadProfiles = yamlConfig.autoLoad?.profiles || [];

          // Look for matching profile in autoLoad
          const matchingProfile = autoLoadProfiles.find(
            (p) => p.name === profileName,
          );
          this.logger.debug(
            "[APPLY-CLAUDE-CONFIG] Searching for matching profile in autoLoad:",
            profileName,
          );
          if (matchingProfile?.path) {
            this.logger.debug(
              "[APPLY-CLAUDE-CONFIG] Found matching profile in autoLoad:",
              matchingProfile.path,
            );
            // Use PathResolver to handle all path resolution
            const pathResolver = PathResolver.getInstance();
            let resolvedPath = pathResolver.resolveProfilePath(
              matchingProfile.path,
            );

            try {
              this.logger.debug(
                "[APPLY-CLAUDE-CONFIG] Attempting to load config from resolved path:",
                resolvedPath,
              );
              await this.claudeConfigManager.loadClaudeConfig(resolvedPath);
              filePath = resolvedPath;
              this.logger.debug(
                "[APPLY-CLAUDE-CONFIG] Successfully loaded from YAML autoLoad path:",
                resolvedPath,
              );
            } catch (error) {
              this.logger.debug(
                "[APPLY-CLAUDE-CONFIG] Failed to load from YAML autoLoad path:",
                resolvedPath,
                "Error:",
                error,
              );
          ...
          this.logger.debug(
            "[APPLY-CLAUDE-CONFIG] Failed to load YAML config:",
            error,
          );
      ...
      if (!filePath) {
        this.logger.debug(
          "[APPLY-CLAUDE-CONFIG] Still no filePath, trying common paths fallback",
        );
        const pathResolver = PathResolver.getInstance();
        const possiblePaths = pathResolver.generateProfilePaths(profileName);
        this.logger.debug(
          "[APPLY-CLAUDE-CONFIG] Generated possible paths:",
          possiblePaths,
        );
        ...
        for (const possiblePath of possiblePaths) {
          try {
            this.logger.debug(
              "[APPLY-CLAUDE-CONFIG] Trying possible path:",
              possiblePath,
            );
            await this.claudeConfigManager.loadClaudeConfig(possiblePath);
            filePath = possiblePath;
            this.logger.debug(
              "[APPLY-CLAUDE-CONFIG] Successfully loaded from possible path:",
              possiblePath,
            );
            break;
          } catch (error) {
            this.logger.debug(
              "[APPLY-CLAUDE-CONFIG] Failed to load from possible path:",
              possiblePath,
              "Error:",
              error,
            );
    ...
    if (filePath) {
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] Resolving filePath format:",
        filePath,
      );
      ...
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] Resolved path:",
        originalPath,
        "to:",
        filePath,
      );
    ...
    if (!filePath) {
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] No filePath found after all resolution attempts",
      );
      return createStandardErrorResponse("File path is required");
    }

    this.logger.debug(
      "[APPLY-CLAUDE-CONFIG] Final filePath to load:",
      filePath,
    );
    ...
    if (!path.extname(filePath) && !filePath.includes("/")) {
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] FilePath has no extension, trying common patterns",
      );
      const pathResolver = PathResolver.getInstance();
      const possiblePaths = pathResolver.generateFilePaths(filePath);
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] Generated file paths:",
        possiblePaths,
      );
      ...
      for (const possiblePath of possiblePaths) {
        try {
          this.logger.debug(
            "[APPLY-CLAUDE-CONFIG] Trying file path:",
            possiblePath,
          );
          await this.claudeConfigManager.loadClaudeConfig(possiblePath);
          filePath = possiblePath;
          this.logger.debug(
            "[APPLY-CLAUDE-CONFIG] Successfully loaded from file path:",
            possiblePath,
          );
          break;
        } catch (error) {
          this.logger.debug(
            "[APPLY-CLAUDE-CONFIG] Failed to load from file path:",
            possiblePath,
            "Error:",
            error,
          );
    ...
    this.logger.debug(
      "[APPLY-CLAUDE-CONFIG] Attempting final config load from:",
      filePath,
    );
    ...
    this.logger.debug(
      "[APPLY-CLAUDE-CONFIG] Successfully loaded config, keys:",
      Object.keys(config),
    );

    // Auto-generate profile name from filename (without extension) or use default
    const yamlConfigManager = new YamlConfigManager();
    const autoProfileName =
      profileName ||
      path.basename(filePath, path.extname(filePath)) ||
      yamlConfigManager.getDefaultProfile();
    this.logger.debug(
      "[APPLY-CLAUDE-CONFIG] Generated profile name:",
      autoProfileName,
    );
    this.activeProfiles.set(autoProfileName, config);
    this.logger.debug(
      "[APPLY-CLAUDE-CONFIG] Stored in activeProfiles, total profiles:",
    ...
    this.lastAppliedTime = new Date().toISOString();
    this.logger.debug(
      "[APPLY-CLAUDE-CONFIG] Tracked last applied profile:",
      autoProfileName,
      "at:",
      this.lastAppliedTime,
    );
    ...
    if (autoApply) {
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] Auto-apply enabled, generating behavior instructions",
      );
      const behaviorInstructions =
        BehaviorGenerator.generateInstructions(config);
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] Generated instructions length:",
        behaviorInstructions.length,
      );

      // Actually apply the behavior instructions to the system
      if (
        this.omniMCPServer &&
        typeof this.omniMCPServer.setAppliedBehaviorInstructions === "function"
      ) {
        this.omniMCPServer.setAppliedBehaviorInstructions(behaviorInstructions);
        this.logger.info(
          "[APPLY-CLAUDE-CONFIG] Behavior instructions applied to system",
        );
      }
      ...
      this.logger.debug(
        "[APPLY-CLAUDE-CONFIG] Auto-apply disabled, showing config summary",
      );
      responseMessages.push({
        type: "text" as const,
        text: `Configuration includes: ${Object.keys(config)
  ...
  private async doHandleListClaudeConfigs(_args: unknown) {
    this.logger.info("[LIST-CLAUDE-CONFIGS] Starting list operation");
    // Get loaded configs
    const loadedConfigNames = Array.from(this.activeProfiles.keys());
    this.logger.info(
      "[LIST-CLAUDE-CONFIGS] Loaded config names:",
      loadedConfigNames,
    );

    // Only show loaded configs, no file scanning

    const result = {
      loaded: loadedConfigNames.map((name) => {
        const filePath =
          ((this.activeProfiles.get(name) as Record<string, unknown>)
            ?._filePath as string) || "unknown";
        const fileName =
          filePath !== "unknown" ? filePath.split("/").pop() : "unknown";
        return {
          name,
          status: "loaded",
          file: fileName,
      ...
      available: [], // No scanning for available files
      summary: {
        totalLoaded: loadedConfigNames.length,
        totalAvailable: 0,
        total: loadedConfigNames.length,
      },
    };
  ...
  private async doHandleGetAppliedConfig(_args: unknown) {
    this.logger.debug(
      "[GET-APPLIED-CONFIG] Starting get applied config operation",
    );
    ...
    this.logger.debug(
      "[GET-APPLIED-CONFIG] Last applied profile:",
      lastAppliedProfile,
    );

    if (!lastAppliedProfile) {
      this.logger.debug(
        "[GET-APPLIED-CONFIG] No configuration currently applied",
      );
      return {
        content: [
          {
    ...
    if (!config) {
      this.logger.debug(
        "[GET-APPLIED-CONFIG] Configuration no longer in memory:",
        lastAppliedProfile,
      );
      return {
        content: [
    ...
    this.logger.debug(
      "[GET-APPLIED-CONFIG] Found configuration for profile:",
      lastAppliedProfile,
    );
    ...
    const profileInfo = {
      ...
      path:
        ((config as Record<string, unknown>)._filePath as string) || "unknown",
