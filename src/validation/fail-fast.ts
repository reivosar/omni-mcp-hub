...
export class FailFastValidator {
  ...
  async validateStartup(
    ...
    try {
      ...
      this.logger.error(
        chalk.red(`CRITICAL Fatal configuration error: ${errorMessage}`),
      );
  ...
  private displayErrors(result: ValidationResult, detailed: boolean): void {
    ...
    result.errors.forEach((error, index) => {
      ...
      if (error.suggestedFix) {
        this.logger.error(chalk.cyan(`   INFO 修正案: ${error.suggestedFix}`));
      }

      if (detailed && error.value !== undefined) {
        this.logger.error(
          chalk.gray(`   現在の値: ${JSON.stringify(error.value)}`),
        );
      }
    });
    ...
    this.logger.error(
      chalk.red(
        `Found ${result.errors.length} error(s) that must be fixed before startup.`,
      ),
    );
  }
  ...
  private displayWarnings(result: ValidationResult, _detailed: boolean): void {
    ...
    result.warnings.forEach((warning, index) => {
      this.logger.warn(
        chalk.yellow(`${index + 1}. ${warning.field}: ${warning.message}`),
      );

      if (warning.suggestedFix) {
        this.logger.warn(chalk.cyan(`   INFO 推奨: ${warning.suggestedFix}`));
      }
    });
  }
  ...
  private displayConfigSummary(result: ValidationResult): void {
    ...
    if (config.externalServers?.enabled && serversCount > 0) {
      this.logger.info(
        chalk.blue(`   External Servers: ${serversCount} configured`),
      );
    }
  ...
  async generateDoctorReport(configPath: string): Promise<string> {
    try {
      ...
      if (result.valid) {
        ...
        report += chalk.red("ERROR Status: REQUIRES ATTENTION\n");
      }
      ...
      if (result.warnings.length > 0) {
        ...
        result.warnings.forEach((warning, index) => {
          ...
          if (warning.suggestedFix) {
            report += `   INFO Suggestion: ${warning.suggestedFix}\n`;
          }
          report += "\n";
        });
      }
      ...
      if (result.valid && result.warnings.length === 0) {
        ...
        report += chalk.yellow(
          "NEW Configuration is valid but could be optimized.\n",
        );
      } else {
      ...
      return chalk.red(
        `CRITICAL Doctor failed to analyze configuration: ${error}`,
      );
    }
  }
}
 ...
 */
export async function validateConfigOnStartup(
  options: FailFastOptions = {},
): Promise<ValidationResult> {
