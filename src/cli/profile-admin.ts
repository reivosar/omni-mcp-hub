...
class ProfileAdminCLI {
  ...
  public addProfile(name: string, profilePath: string, permissions?: string[]): void {
  ...
  public updateProfile(name: string, newPath?: string, permissions?: string[]): void {
    const profile = this.profiles.get(name);
...
program
  ...
  .option('-p, --permissions <perms...>', 'Set permissions (read, write, execute, admin)')
