...
describe('RESTServer', () => {
  ...
  beforeEach(() => {
    ...
    (mockCrypto.createHmac as jest.Mock) = jest.fn(() => mockHmac as any);
    (mockCrypto.timingSafeEqual as jest.Mock) = jest.fn(() => true);
  ...
  describe('constructor', () => {
    it('should initialize with default port', () => {
      const defaultServer = new RESTServer();
  ...
  describe('MCP SSE endpoint', () => {
    ...
    it('should handle successful MCP data request with cache hit', async () => {
      ...
      const mcpHandler = mockApp.get.mock.calls.find((call: any) => call[0] === '/:owner/:repo/sse')[1];
    ...
    it('should handle external references when include_externals is true', async () => {
      ...
      // Mock fetch for external content
  ...
  describe('files endpoint', () => {
    ...
    it('should list files successfully', async () => {
      ...
      const filesHandler = mockApp.get.mock.calls.find((call: any) => call[0] === '/:owner/:repo/files')[1];
  ...
  describe('raw file proxy endpoint', () => {
    ...
    it('should serve raw file with correct content type for markdown', async () => {
      ...
      const rawHandler = mockApp.get.mock.calls.find((call: any) => call[0] === '/:owner/:repo/raw/*')[1];
  ...
  describe('webhook endpoint', () => {
    ...
    it('should handle push webhook with valid signature', async () => {
      mockReq.get.mockImplementation((header: string) => {
        ...
        if (header === 'X-Hub-Signature-256') return 'sha256=valid-signature';
        if (header === 'X-GitHub-Delivery') return 'delivery-id';
        return undefined;
      });
      ...
      const webhookHandler = mockApp.post.mock.calls.find((call: any) => call[0] === '/webhook/github')[1];
    ...
    it('should handle push webhook without signature when no secret configured', async () => {
      mockReq.get.mockImplementation((header: string) => {
        if (header === 'X-GitHub-Event') return 'push';
        if (header === 'X-Hub-Signature-256') return undefined;
        return undefined;
      });
    ...
    it('should handle repository webhook', async () => {
      mockReq.get.mockImplementation((header: string) => {
        if (header === 'X-GitHub-Event') return 'repository';
        return undefined;
      });
    ...
    it('should ignore unknown webhook events', async () => {
      mockReq.get.mockImplementation((header: string) => {
        if (header === 'X-GitHub-Event') return 'unknown_event';
        return undefined;
      });
  ...
  describe('config endpoint', () => {
    it('should return configuration', () => {
      const configHandler = mockApp.get.mock.calls.find((call: any) => call[0] === '/config')[1];
      const mockReq = {};
      const mockRes = {
        json: jest.fn()
      };
  ...
  describe('version endpoint', () => {
    it('should return version information', () => {
      const versionHandler = mockApp.get.mock.calls.find((call: any) => call[0] === '/version')[1];
      const mockReq = {};
      const mockRes = {
        json: jest.fn()
      };
  ...
  describe('request counting middleware', () => {
    it('should increment request count', () => {
      // Get the request counter middleware
      const middlewareCalls = mockApp.use.mock.calls;
      const counterMiddleware = middlewareCalls.find((call: any) => 
  ...
  describe('extractExternalReferences', () => {
    it('should extract HTTP URLs', () => {
      ...
      const refs = (server as any).extractExternalReferences(content);
  ...
  describe('fetchExternalContent', () => {
    it('should fetch HTTP content', async () => {
      ...
      const content = await (server as any).fetchExternalContent('https://example.com/docs.md');
    ...
    it('should handle HTTP errors', async () => {
      ...
      await expect((server as any).fetchExternalContent('https://example.com/missing.md'))
    ...
    it('should fetch GitHub content', async () => {
      ...
      const content = await (server as any).fetchExternalContent('github:owner/repo/docs/guide.md');
    ...
    it('should throw error for unsupported reference format', async () => {
      await expect((server as any).fetchExternalContent('ftp://example.com/file.md'))
  ...
  describe('verifyWebhookSignature', () => {
    it('should verify valid signature', () => {
      ...
      const isValid = (server as any).verifyWebhookSignature(payload, signature, secret);
    ...
    it('should handle Buffer payload', () => {
      ...
      (server as any).verifyWebhookSignature(payload, signature, secret);
