...
export class MonitoringServer {
  ...
  async start(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.server.listen(this.config.port, this.config.host, () => {
        this.logger.info(
          `Monitoring server listening on ${this.config.host}:${this.config.port}`,
        );
        resolve();
      });
    });
    ...
    const stats = {
      ...
      tools: {
        ...
        averageExecutionTime:
          Math.round(metrics.averageToolExecutionTime) + "ms",
      ...
      errors: {
        ...
        totalErrors: Object.values(metrics.errorsByType).reduce(
          (a, b) => a + b,
          0,
        ),
  ...
  private generateTextDashboard(
    metrics: ReturnType<MetricsCollector["getCurrentMetrics"]>,
  ): string {
    const successRate =
      metrics.totalRequests > 0
        ? (
...
${metrics.topTools
  .slice(0, 3)
  .map(
    (tool: { name: string; count: number; avgTime: number }, i: number) =>
