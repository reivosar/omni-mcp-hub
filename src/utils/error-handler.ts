...
export class ErrorHandler {
    ...
    try {
      if (context?.timeout) {
        const timeoutPromise = new Promise<never>((_, reject) => {
          setTimeout(() => reject(new Error(`Operation timeout after ${context.timeout}ms`)), context.timeout);
        });
  ...
  isRetryableError(error: Error): boolean {
    ...
    const retryableMessages = ['ECONNREFUSED', 'ETIMEDOUT', 'timeout', 'network'];
  ...
  getErrorStatistics(): ErrorStatistics {
    ...
    if (this.errorHistory.length > 0) {
      stats.lastErrorTime = this.errorHistory[this.errorHistory.length - 1].timestamp;
    }
...
`.trim();
  ...
  logOperationSuccess(context: MCPErrorContext, duration?: number): void {
    ...
    try {
      ...
      console.error('Logger error during success metrics logging:', _loggerError);
    }
  }
}
