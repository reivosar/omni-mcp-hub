import { AuditLogger, AuditLogEntry } from '../../../src/security/audit-logger';
import { CommandExecution } from '../../../src/security/command-validator';
...
describe('AuditLogger', () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(mockFs, 'statSync').mockImplementation(() => ({ size: 1000 } as any));
  ...
  describe('Log Writing', () => {
    ...
    test('should log validation attempts', () => {
      ...
      expect(mockFs.appendFileSync).toHaveBeenCalledWith(
        ...
        expect.stringContaining('"event":"COMMAND_VALIDATION_ATTEMPT"'),
        'utf8'
      );
    });
    ...
    test('should log validation success', () => {
      ...
      expect(mockFs.appendFileSync).toHaveBeenCalledWith(
        expect.stringContaining('security-audit.log'),
        expect.stringContaining('"event":"COMMAND_VALIDATION_SUCCESS"'),
        'utf8'
      );
    });
    ...
    test('should log command execution', () => {
      ...
      expect(mockFs.appendFileSync).toHaveBeenCalledWith(
        expect.stringContaining('security-audit.log'),
        expect.stringContaining('"event":"COMMAND_EXECUTED"'),
        'utf8'
      );
    ...
    test('should log command failures', () => {
      ...
      expect(mockFs.appendFileSync).toHaveBeenCalledWith(
        expect.stringContaining('security-audit.log'),
        expect.stringContaining('Process spawn failed'),
        'utf8'
      );
    });
    ...
    test('should log configuration validation', () => {
      ...
      expect(mockFs.appendFileSync).toHaveBeenCalledWith(
        expect.stringContaining('security-audit.log'),
        expect.stringContaining('"event":"CONFIGURATION_VALIDATION"'),
        'utf8'
      );
    ...
    test('should log policy updates', () => {
      ...
      expect(mockFs.appendFileSync).toHaveBeenCalledWith(
        expect.stringContaining('security-audit.log'),
        expect.stringContaining('"event":"SECURITY_POLICY_UPDATE"'),
        'utf8'
      );

      expect(mockFs.appendFileSync).toHaveBeenCalledWith(
        expect.stringContaining('security-audit.log'),
        expect.stringContaining('"maxArguments":100'),
        'utf8'
      );
    });
    ...
    test('should log suspicious activity', () => {
      ...
      expect(mockFs.appendFileSync).toHaveBeenCalledWith(
        expect.stringContaining('security-audit.log'),
        expect.stringContaining('Multiple failed validation attempts'),
        'utf8'
      );
    });
  ...
  describe('Log Rotation', () => {
    test('should rotate logs when size limit exceeded', () => {
      ...
      jest.spyOn(mockFs, 'statSync').mockReturnValue({ size: 15 * 1024 * 1024 } as any); // 15MB > 10MB limit
    ...
    test('should delete oldest log files during rotation', () => {
      jest.spyOn(mockFs, 'existsSync').mockImplementation((path) => {
        ...
        // The main log file exists (to trigger rotation) and the oldest (.log.4) exists to be deleted
        return pathStr.includes('security-audit.log') && !pathStr.includes('.log.5');
      });
      jest.spyOn(mockFs, 'statSync').mockReturnValue({ size: 15 * 1024 * 1024 } as any);
  ...
  describe('Security Violation Filtering', () => {
    test('should filter security violations by time', () => {
      ...
      const violations = auditLogger.getSecurityViolations(1); // Last 1 hour
