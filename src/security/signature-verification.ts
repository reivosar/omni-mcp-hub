...
export class ProfileSignatureVerifier {
  ...
  public verifyChecksum(
    ...
    if (
      ...
      !hexPattern.test(expectedChecksum)
    ) {
      return false;
    }
  ...
  public createBundle(
    ...
    profiles: { [name: string]: SignedProfile };
    bundleSignature: SignedProfile;
    bundleMetadata: { created: number; profileCount: number; version: string };
  } {
    const bundle = {
    ...
    const bundleSignature = this.signProfile(
      bundleContent,
      privateKeyPem,
      keyId,
    );
   ...
   */
  public verifyBundle(bundle: {
    profiles: { [name: string]: SignedProfile };
    bundleSignature?: SignedProfile;
  }): {
    isValid: boolean;
    results: { [name: string]: VerificationResult };
