...
const simple_server_1 = require("../../../src/servers/simple-server");
...
describe('SimpleMCPServer', () => {
    ...
    beforeEach(() => {
        ...
        MockWebSocket.Server = jest.fn().mockImplementation(() => mockWSServer);
    ...
    describe('constructor', () => {
        it('should initialize WebSocket server with default port', () => {
            delete process.env.MCP_PORT;
            server = new simple_server_1.SimpleMCPServer();
    ...
    describe('initialize', () => {
        ...
        it('should load GitHub source successfully', async () => {
            ...
            expect(mockGit.clone).toHaveBeenCalledWith('https://github.com/anthropics/claude-code.git', '/app/repos/github-anthropics-claude-code', ['--depth', '1']);
        ...
        it('should handle local source loading failures', async () => {
            mockFs.existsSync.mockImplementation((path) => {
                if (path.includes('lum'))
                    throw new Error('Lum source error');
                if (path.includes('rerere'))
                    throw new Error('Rerere source error');
                return false;
            });
            await server.initialize();
    ...
    describe('loadFilesFromDirectory', () => {
        ...
        it('should return empty map for non-existent directory', () => {
            mockFs.existsSync.mockReturnValue(false);
            const result = server.loadFilesFromDirectory('/nonexistent');
            expect(result.size).toBe(0);
        });
        it('should load files with supported extensions', () => {
            mockFs.existsSync.mockReturnValue(true);
            mockFs.readdirSync.mockReturnValue(['test.md', 'config.json', 'data.yaml', 'readme.txt']);
            mockFs.statSync.mockReturnValue({ isDirectory: () => false });
            mockFs.readFileSync.mockReturnValue('file content');
            const result = server.loadFilesFromDirectory('/test');
        ...
        it('should skip unsupported file extensions', () => {
            mockFs.existsSync.mockReturnValue(true);
            mockFs.readdirSync.mockReturnValue(['script.py', 'binary.exe', 'image.png']);
            mockFs.statSync.mockReturnValue({ isDirectory: () => false });
            const result = server.loadFilesFromDirectory('/test');
            expect(result.size).toBe(0);
        });
        ...
        it('should recursively load directories', () => {
            mockFs.existsSync.mockReturnValue(true);
            mockFs.readdirSync.mockReturnValue(['root.md']);
            mockFs.statSync.mockReturnValue({ isDirectory: () => false });
    ...
    describe('WebSocket message handling', () => {
        ...
        beforeEach(async () => {
            ...
            await server.initialize();
            connectionHandler = mockWSServer.on.mock.calls.find((call) => call[0] === 'connection')[1];
        });
    ...
    describe('tool handling', () => {
        ...
        it('should handle list_sources tool', () => {
            const result = server.listSources();
        ...
        it('should handle get_file_variants tool', () => {
            const result = server.getFileVariants('README.md');
        ...
        it('should handle get_file_variants for non-existent file', () => {
            expect(() => server.getFileVariants('nonexistent.md'))
                .toThrow('File nonexistent.md not found in any source');
        });
        it('should handle get_source_file tool', () => {
            const result = server.getSourceFile('github:test/repo', 'config.json');
        ...
        it('should handle get_source_file for non-existent source', () => {
            expect(() => server.getSourceFile('nonexistent', 'file.md'))
                .toThrow('Source not found: nonexistent');
        });
        it('should handle get_source_file for non-existent file', () => {
            expect(() => server.getSourceFile('github:test/repo', 'nonexistent.md'))
    ...
    describe('loadGitHubSource', () => {
        ...
        it('should clone repository if not exists', async () => {
            mockFs.existsSync.mockReturnValue(false);
            mockFs.readdirSync.mockReturnValue([]);
            await server.loadGitHubSource('user/repo');
            expect(mockGit.clone).toHaveBeenCalledWith('https://github.com/user/repo.git', '/app/repos/github-user-repo', ['--depth', '1']);
        });
    ...
    describe('loadLocalSource', () => {
        ...
        it('should load local source with correct name', async () => {
            mockFs.existsSync.mockReturnValue(true);
            mockFs.readdirSync.mockReturnValue([]);
            const result = await server.loadLocalSource('/test/path', 'test-source');
