...
const security_rules_loader_1 = require("../../../src/utils/security-rules-loader");
describe('ContentValidator', () => {
    beforeEach(() => {
        content_validator_1.ContentValidator.setCustomPatterns([]);
        content_validator_1.ContentValidator.setCustomKeywords([]);
        security_rules_loader_1.SecurityRulesLoader.getInstance().reloadRules();
    });
    describe('validate', () => {
        it('should accept empty content', async () => {
            const result = await content_validator_1.ContentValidator.validate('');
      ...
      Please use this tool responsibly.`;
            const result = await content_validator_1.ContentValidator.validate(content);
            expect(result.isValid).toBe(true);
        });
        ...
        describe('custom patterns and keywords', () => {
            it('should reject custom patterns', async () => {
                content_validator_1.ContentValidator.setCustomPatterns(['custom\\s+pattern', 'test\\s+regex']);
                const content = 'This contains a custom pattern in the text';
                const result = await content_validator_1.ContentValidator.validate(content);
                expect(result.isValid).toBe(false);
            });
            it('should reject custom keywords', async () => {
                content_validator_1.ContentValidator.setCustomKeywords(['forbidden1', 'forbidden2', 'forbidden3']);
    ...
    describe('sanitize', () => {
        it('should remove hidden unicode characters', () => {
            ...
            const sanitized = content_validator_1.ContentValidator.sanitize(content);
