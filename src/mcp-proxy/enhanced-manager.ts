...
export class EnhancedMCPProxyManager extends MCPProxyManager {
  ...
  constructor(
    ...
    this.enhancedLogger.info(
      "[ENHANCED-MGR] Enhanced MCP Proxy Manager initialized",
    );
  }
  ...
  private async addEnhancedServer(config: ExternalServerConfig): Promise<void> {
    this.enhancedLogger.info(
      `[ENHANCED-MGR] Adding enhanced server: ${config.name}`,
    );

    try {
      await this.resilienceManager.addServer(
        config,
      ...
      this.enhancedLogger.error(
        `[ENHANCED-MGR] Failed to add enhanced server ${config.name}:`,
        error,
      );
  ...
  private async addLegacyServer(config: ExternalServerConfig): Promise<void> {
    this.enhancedLogger.info(
      `[ENHANCED-MGR] Adding legacy server: ${config.name}`,
    );
    ...
    try {
      ...
      this.enhancedLogger.info(
        `[ENHANCED-MGR] Legacy server added successfully: ${config.name}`,
      );
    } catch (error) {
      this.enhancedLogger.error(
        `[ENHANCED-MGR] Failed to add legacy server ${config.name}:`,
        error,
      );
      throw error;
    }
  ...
  async removeServer(name: string): Promise<void> {
    ...
    if (isEnhanced) {
      this.enhancedLogger.info(
        `[ENHANCED-MGR] Removing enhanced server: ${name}`,
      );
      await this.resilienceManager.removeServer(name);
      this.enhancedServers.delete(name);
    } else {
      this.enhancedLogger.info(
        `[ENHANCED-MGR] Removing legacy server: ${name}`,
      );
  ...
  async callTool(toolName: string, args: unknown): Promise<CallToolResult> {
    ...
    if (isEnhanced && this.config.features.loadBalancing) {
      this.enhancedLogger.debug(
        `[ENHANCED-MGR] Calling tool ${toolName} with resilience`,
      );
      return (await this.resilienceManager.callTool(
        toolName,
        args,
        serverName,
      )) as CallToolResult;
      ...
      this.enhancedLogger.debug(
        `[ENHANCED-MGR] Calling tool ${toolName} in legacy mode`,
      );
  ...
  async readResource(uri: string): Promise<ReadResourceResult> {
    ...
    if (isEnhanced && this.config.features.loadBalancing) {
      this.enhancedLogger.debug(
        `[ENHANCED-MGR] Reading resource ${uri} with resilience`,
      );
      return (await this.resilienceManager.readResource(
        uri,
        serverName,
      )) as ReadResourceResult;
    } else {
      this.enhancedLogger.debug(
        `[ENHANCED-MGR] Reading resource ${uri} in legacy mode`,
      );
  ...
  async performHealthCheck(): Promise<Map<string, boolean>> {
    ...
    if (this.config.features.healthChecking) {
      ...
      for (const stats of serverStats) {
        enhancedHealth.set(
          stats.serverName,
          stats.healthCheckStatus === "healthy",
        );
      }
    }
  ...
  async migrateServerToEnhanced(serverName: string): Promise<void> {
    ...
    try {
      ...
      this.enhancedLogger.info(
        `[ENHANCED-MGR] Successfully migrated ${serverName} to enhanced mode`,
      );
    } catch (error) {
      this.enhancedLogger.error(
        `[ENHANCED-MGR] Failed to migrate ${serverName}:`,
        error,
      );
      ...
      try {
        ...
        this.enhancedLogger.error(
          `[ENHANCED-MGR] Failed to restore legacy mode for ${serverName}:`,
          restoreError,
        );
      }
  ...
  async migrateServerToLegacy(serverName: string): Promise<void> {
    ...
    try {
      ...
      this.enhancedLogger.info(
        `[ENHANCED-MGR] Successfully migrated ${serverName} to legacy mode`,
      );
    } catch (error) {
      this.enhancedLogger.error(
        `[ENHANCED-MGR] Failed to migrate ${serverName}:`,
