...
const DEFAULT_RESILIENCE_CONFIG: ResilienceConfig = {
  ...
  degradedMode: {
    ...
    fallbackStrategies: ['cached_response', 'default_response', 'error_response']
  }
};
...
class CircuitBreaker {
  ...
  async execute<T>(operation: () => Promise<T>, serverName: string): Promise<T> {
    if (this.state === 'open') {
      if (Date.now() < this.nextAttempt) {
        throw new Error(`Circuit breaker is OPEN for ${serverName}. Next attempt in ${this.nextAttempt - Date.now()}ms`);
      }
      // Transition to half-open
      this.state = 'half_open';
      this.halfOpenAttempts = 0;
      this.logger.info(`[CIRCUIT-BREAKER] ${serverName}: Transitioning to HALF_OPEN state`);
    }
  ...
  private onSuccess(serverName: string): void {
    ...
    if (this.state !== 'closed') {
      this.state = 'closed';
      this.logger.info(`[CIRCUIT-BREAKER] ${serverName}: Circuit closed after successful operation`);
    }
  }
  ...
  private openCircuit(serverName: string): void {
    ...
    this.logger.warn(`[CIRCUIT-BREAKER] ${serverName}: Circuit OPENED after ${this.failureCount} failures`);
  }
...
export class ResilientMCPConnection extends EventEmitter {
  ...
  private connectionHistory: Array<{ timestamp: Date; success: boolean; duration: number }> = [];
  ...
  private setupEventListeners(): void {
    ...
    this.on('circuitBreakerStateChange', (newState) => {
      ...
      this.updateConnectionState(newState === 'open' ? ConnectionState.CIRCUIT_OPEN : this.state);
    });
  }
  ...
  async connect(): Promise<void> {
    ...
    for (let attempt = 0; attempt < this.config.maxRetryAttempts; attempt++) {
      ...
      try {
        await this.circuitBreaker.execute(async () => {
          await Promise.race([
            this.client.connect(),
            this.createTimeoutPromise(this.config.connection.connectTimeoutMs, 'Connection timeout')
          ]);
        }, this.stats.serverName);
        
        // Connection successful
        const duration = Date.now() - startTime;
        this.connectionHistory.push({ timestamp: new Date(), success: true, duration });
        ...
        this.logger.info(`[RESILIENT-MCP] Successfully connected to ${this.stats.serverName} (attempt ${attempt + 1}/${this.config.maxRetryAttempts})`);
        return;
        
      } catch (error) {
        lastError = error as Error;
        const duration = Date.now() - startTime;
        this.connectionHistory.push({ timestamp: new Date(), success: false, duration });
        this.stats.failedConnections++;
        this.stats.consecutiveFailures++;
        this.stats.lastFailure = new Date();
        this.stats.lastFailureReason = lastError.message;
        
        this.logger.warn(`[RESILIENT-MCP] Connection attempt ${attempt + 1}/${this.config.maxRetryAttempts} failed for ${this.stats.serverName}:`, error);
        
        if (attempt < this.config.maxRetryAttempts - 1) {
          const delay = this.calculateBackoffDelay(attempt);
          this.logger.info(`[RESILIENT-MCP] Retrying connection to ${this.stats.serverName} in ${delay}ms`);
    ...
    throw new Error(`Failed to connect to ${this.stats.serverName} after ${this.config.maxRetryAttempts} attempts. Last error: ${lastError?.message}`);
  }
  ...
  async disconnect(): Promise<void> {
    ...
    if (this.stats.currentOperations > 0) {
      this.logger.info(`[RESILIENT-MCP] Waiting for ${this.stats.currentOperations} operations to complete before disconnecting ${this.stats.serverName}`);
      
      // Wait for operations to complete or timeout
      const maxWait = this.config.connection.gracefulShutdownTimeoutMs;
      const startTime = Date.now();
      
      while (this.stats.currentOperations > 0 && (Date.now() - startTime) < maxWait) {
        await this.sleep(100);
      }
      
      if (this.stats.currentOperations > 0) {
        this.logger.warn(`[RESILIENT-MCP] Force disconnecting ${this.stats.serverName} with ${this.stats.currentOperations} pending operations`);
      }
    }
    ...
    try {
      ...
      this.logger.info(`[RESILIENT-MCP] Gracefully disconnected from ${this.stats.serverName}`);
    } catch (error) {
      this.logger.warn(`[RESILIENT-MCP] Error during disconnect from ${this.stats.serverName}:`, error);
      this.updateConnectionState(ConnectionState.FAILED);
    }
  }
   ...
   */
  private async executeWithResilience<T>(operation: () => Promise<T>, operationId: string): Promise<T> {
    if (this.state !== ConnectionState.CONNECTED && this.state !== ConnectionState.DEGRADED) {
      ...
      throw new Error(`Server ${this.stats.serverName} is not connected (state: ${this.state})`);
    }
    ...
    try {
      const result = await this.circuitBreaker.execute(operation, this.stats.serverName);
      ...
      this.logger.warn(`[RESILIENT-MCP] Operation ${operationId} failed on ${this.stats.serverName}:`, error);
      ...
      try {
        switch (strategy) {
          case 'cached_response': {
            const cached = this.getCachedResponse<T>(operationId);
            if (cached) {
              this.logger.info(`[RESILIENT-MCP] Using cached response for ${operationId} on ${this.stats.serverName}`);
          ...
          case 'default_response': {
            ...
            if (defaultResponse !== null) {
              this.logger.info(`[RESILIENT-MCP] Using default response for ${operationId} on ${this.stats.serverName}`);
          ...
          case 'error_response': {
            ...
            this.logger.info(`[RESILIENT-MCP] Using error response for ${operationId} on ${this.stats.serverName}`);
            return Promise.resolve(errorResponse);
          }
        }
      } catch (fallbackError) {
        this.logger.warn(`[RESILIENT-MCP] Fallback strategy ${strategy} failed for ${operationId}:`, fallbackError);
      }
    }
    
    throw new Error(`All fallback strategies exhausted for ${operationId} on ${this.stats.serverName}`);
  }
  ...
  private startHealthChecks(): void {
    ...
    this.logger.debug(`[RESILIENT-MCP] Started health checks for ${this.stats.serverName} (interval: ${this.config.healthCheck.intervalMs}ms)`);
  }
  ...
  private stopHealthChecks(): void {
    if (this.healthCheckInterval) {
      ...
      this.logger.debug(`[RESILIENT-MCP] Stopped health checks for ${this.stats.serverName}`);
    }
  }
  ...
  private async performHealthCheck(): Promise<boolean> {
    ...
    try {
      ...
      if (!isHealthy) {
        this.logger.warn(`[RESILIENT-MCP] Health check failed for ${this.stats.serverName} using strategy: ${this.config.healthCheck.strategy}`);
        await this.handleHealthCheckFailure();
      } else if (this.state === ConnectionState.DEGRADED && isHealthy) {
        // Recover from degraded state
        this.updateConnectionState(ConnectionState.CONNECTED);
        this.logger.info(`[RESILIENT-MCP] ${this.stats.serverName} recovered from degraded state`);
      }
      
      return isHealthy;
      
    } catch (error) {
      this.logger.error(`[RESILIENT-MCP] Health check error for ${this.stats.serverName}:`, error);
  ...
  private async handleHealthCheckFailure(): Promise<void> {
    ...
    if (this.stats.consecutiveFailures >= this.config.healthCheck.consecutiveFailureThreshold) {
      if (this.config.degradedMode.enabled && this.state === ConnectionState.CONNECTED) {
        this.updateConnectionState(ConnectionState.DEGRADED);
        this.logger.warn(`[RESILIENT-MCP] ${this.stats.serverName} entered degraded mode after ${this.stats.consecutiveFailures} consecutive health check failures`);
      } else if (this.state !== ConnectionState.RECONNECTING) {
        this.logger.warn(`[RESILIENT-MCP] Attempting to reconnect ${this.stats.serverName} after ${this.stats.consecutiveFailures} consecutive health check failures`);
  ...
  private async attemptReconnection(): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`[RESILIENT-MCP] Reconnection failed for ${this.stats.serverName}:`, error);
      this.updateConnectionState(ConnectionState.FAILED);
    }
  }
  ...
  private calculateBackoffDelay(attempt: number): number {
    ...
    const jitter = exponentialDelay * this.config.retryJitterFactor * Math.random();
  ...
  private updateConnectionState(newState: ConnectionState): void {
    ...
    if (oldState !== newState) {
      ...
      this.logger.debug(`[RESILIENT-MCP] ${this.stats.serverName} state changed: ${oldState} -> ${newState}`);
    }
  }
  ...
  private getCachedResponse<T>(operationId: string): T | null {
    ...
    if (Date.now() - cached.timestamp > this.config.degradedMode.cacheResponsesMs) {
  ...
  private updateAverageConnectionTime(): void {
    ...
    if (recentConnections.length > 0) {
      this.stats.averageConnectionTimeMs = recentConnections
        .reduce((sum, h) => sum + h.duration, 0) / recentConnections.length;
    }
  }
  ...
  private updateAverageOperationTime(duration: number): void {
    ...
    this.stats.averageOperationTimeMs = this.stats.averageOperationTimeMs === 0
      ? duration
  ...
  updateConfig(newConfig: Partial<ResilienceConfig>): void {
    ...
    this.logger.info(`[RESILIENT-MCP] Updated resilience config for ${this.stats.serverName}`);
  }
}