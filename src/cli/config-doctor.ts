...
program
  ...
  .action(async () => {
    try {
      ...
      console.log(chalk.yellow('Running comprehensive configuration analysis...\n'));
      ...
      if (result.valid) {
        ...
        result.errors.forEach((error, index) => {
          ...
          if (error.line) {
            console.log(chalk.yellow(`   Location: Line ${error.line}${error.column ? `, Column ${error.column}` : ''}`));
          }
      ...
      if (result.config) {
        ...
        const serversCount = result.config.externalServers?.servers?.length || 0;
        if (result.config.externalServers?.enabled && serversCount > 0) {
          console.log(chalk.blue(`   External Servers: ${serversCount}`));
        }
        
        console.log(chalk.blue(`   Log Level: ${result.config.logging?.level || 'info'}`));
      }
