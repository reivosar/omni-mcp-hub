...
class AdminUI {
  ...
  private async listProfiles(): Promise<void> {
    ...
    this.profiles.forEach((profile) => {
      const status = profile.active ? chalk.green('Active') : chalk.gray('Inactive');
      ...
      table.push([
        profile.name,
        profile.path.length > 35 ? '...' + profile.path.slice(-32) : profile.path,
        status,
        tags,
        modified
      ]);
  ...
  private async addProfile(): Promise<void> {
    ...
    const answers = await inquirer.prompt([
      ...
      },
      {
        ...
        choices: ['development', 'production', 'testing', 'experimental', 'custom']
      },
      {
        type: 'confirm',
        name: 'active',
  ...
  private async removeProfile(): Promise<void> {
    ...
    if (confirm) {
      ...
      this.saveProfiles();
      console.log(chalk.green(`Profile '${profileName}' removed successfully!`));
    } else {
  ...
  private async exportProfiles(): Promise<void> {
    ...
    try {
      ...
      console.log(chalk.green(`Exported ${this.profiles.size} profiles to ${filePath}`));
    } catch (error) {
  ...
  public async showSystemStatus(): Promise<void> {
    ...
    const stats = {
      ...
      yamlManagedProfiles: Array.from(this.profiles.values()).filter(p => p.tags?.includes('yaml-managed')).length,
  ...
  private async showInheritanceChain(): Promise<void> {
    ...
    const profilePaths = Array.from(this.profiles.values()).map(p => ({ name: p.name, value: p.path }));
    ...
    const outputAnswer = await inquirer.prompt([
      {
        ...
        validate: (input: string) => input.trim() ? true : 'Output path is required'
      }
    ]);
  ...
  private async previewResolution(): Promise<void> {
    ...
    try {
      ...
      const sections = ['instructions', 'context', 'rules', 'knowledge', 'tools', 'memory'];
